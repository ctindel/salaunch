...
const dataSort = (data, asc, column) => {
  ...
  switch (column){
    case SortColumns.PHONENUMBER:
      sortedData = data.sort((a, b) => { return a.mobilePhone < b.mobilePhone });
      break;
...
const manipulateData = (state, action) => {
  ...
  switch (action.type) {
    ...
    case types.ADD_CONTACT:
        state.data = dataSort([...state.data, new Contact(action.payload.id,action.payload.firstName,payload.lastName,payload.email,action.payload.mobilePhone)], state.sorting.asc, state.sorting.column);
        if(state.searched)
    ...
    case types.INPUT_CONTACT_EMAIL:
    return action.payload.type ? { 
      ...state,
      modal : {
        ...state.modal,
      ...
      ...state,
      input: { ...state.input, email: action.payload.value }
    };
    ...
    return action.payload.type ? {
      ...
      ...state,
      input: { ...state.input, mobilePhone: action.payload.value }
    };
...
const filterSearchResult = (data, searchParam) =>  data.filter(x => x.lastName.toLowerCase().indexOf(searchParam.toLowerCase()) !== -1 || (`${x.mobilePhone}`).indexOf(searchParam) !== -1 );
