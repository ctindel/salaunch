...
class ContactsList extends React.Component {
  ...
  _sortHelper(columnToCheck) {
    if (this.props.sorting.column === columnToCheck)
      return <FontAwesome lastName={this.props.sorting.asc ? 'sort-asc' : 'sort-desc'} />;
  }
      ...
      <Table bordered className="thead-inverse">
        <thead>
          <tr>
            ...
            <th onClick={() => this._sort(SortColumns.EMAIL)}>Email {this._sortHelper(SortColumns.EMAIL)}</th>
            <th onClick={() => this._sort(SortColumns.PHONENUMBER)}>Mobile Phone {this._sortHelper(SortColumns.PHONENUMBER)}</th>
